...
public class PageResolver {
	...
	public void doPaging(int totalCnt, SearchItem sc) {
		this.totalPage = totalCnt / sc.getPageSize() + (totalCnt % sc.getPageSize() == 0 ? 0 : 1); //(페이지 내비게이션의)전체 페이지 갯수
		this.sc.setPage(Math.min(sc.getPage(), totalPage));		//현재page가 totalPage보다 크지 않음 //전체 페이지 갯수(totalPage)가 10이라 할 때 현재page는 1~10사이여야 함
